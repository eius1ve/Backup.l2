...
public class EffectManaHeal
extends Effect {
    private final boolean fH;

    public EffectManaHeal(Env env, EffectTemplate effectTemplate) {
        super(env, effectTemplate);
        this.fH = effectTemplate.getParam().getBool("ignoreMpEff", false);
    }
    ...
    public void onStart() {
        ...
        double d = this.calc();
        double d2 = Math.min(d * 1.7, d * (!this.fH ? this._effected.calcStat(Stats.MANAHEAL_EFFECTIVNESS, 100.0, this._effector, this.getSkill()) : 100.0) / 100.0);
