    ...
    protected void runImpl() {
        ...
        block44: {
            ...
            if (player2.isFishing()) {
                player2.sendPacket((IStaticPacket)SystemMsg.YOU_CANNOT_DO_THAT_WHILE_FISHING_);
                return;
            }
            player = (Player)player2.getVisibleObject(this.rQ);
            if (player == null || player.getPrivateStoreType() != 1 && player.getPrivateStoreType() != 8 || !player.isInRangeZ(player2, 200L)) {
            ...
            try {
                block24: for (int i = 0; i < this.gT; ++i) {
                    ...
                    for (TradeItem tradeItem2 : list) {
                        ...
                        if (itemInstance == null || itemInstance.getCount() < l3) break block44;
                ...
                arrayList.clear();
                player2.sendPacket((IStaticPacket)SystemMsg.INCORRECT_ITEM_COUNT);
                return;
            }
            finally {
                try {
                    ...
                    for (TradeItem tradeItem : arrayList) {
                        ...
                        Log.LogItem(player, Log.ItemLog.PrivateStoreSell, itemInstance, itemInstance.getCount(), tradeItem.getOwnersPrice());
                        Log.LogItem(player2, Log.ItemLog.PrivateStoreBuy, itemInstance, itemInstance.getCount(), tradeItem.getOwnersPrice());
                        player.sendPacket((IStaticPacket)new ExPrivateStoreSellingResult(tradeItem.getObjectId(), tradeItem.getCount(), player2.getName()));
