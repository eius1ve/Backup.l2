...
public class GiveAllByHwidCommand
implements ITelegramCommandHandler {
    private final String[] P = new String[]{"/give_all_by_hwid"};
    ...
    public void handle(String string, String string2) throws Exception {
        ...
        if (string2 == null || string2.trim().isEmpty()) {
            TelegramApi.sendForceReplyMessage(string, "Usage: /give_all_by_hwid <id> <count>", "Enter parameters in format: <id> <count>");
            return;
        }
        String[] stringArray = string2.split(" ", 2);
        if (stringArray.length < 2 || stringArray[0].isEmpty() || stringArray[1].isEmpty()) {
            TelegramApi.sendMessage(string, "Usage: /give_all_by_hwid <id> <count>");
            return;
        }
        try {
        ...
        for (Player object : GameObjectsStorage.getAllPlayersForIterate()) {
            ...
            if (object == null || object2 == null || object.isInOfflineMode() || object.isLogoutStarted() || ((GameClient)object2).getHwid() == null) continue;
            hashMap.putIfAbsent(((GameClient)object2).getHwid(), object.getRef());
        }
        int n2 = 0;
        ...
        TelegramApi.sendMessage(string, "Rewarded " + n2 + " players with " + l + " " + itemTemplate.getName() + " based on unique HWIDs.");
    }
