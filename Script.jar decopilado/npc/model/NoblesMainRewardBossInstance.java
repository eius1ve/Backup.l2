...
public class NoblesMainRewardBossInstance
extends BossInstance {
    public NoblesMainRewardBossInstance(int n, NpcTemplate npcTemplate) {
        super(n, npcTemplate);
    }
    ...
    protected void onDeath(Creature creature) {
        ...
        if (creature != null && creature.isPlayable()) {
            ...
            for (Player player2 : party) {
                if (player2 == null || player2.isDead() || player2.isNoble() || player2.isSubClassActive() || player2.getLevel() < 76 || !CategoryData.fourth_class_group.isPlayerBaseClassBelong(player2) || this.getDistance3D((GameObject)player2) > (double)Config.ALT_PARTY_DISTRIBUTION_RANGE) continue;
