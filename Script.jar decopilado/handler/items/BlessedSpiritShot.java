...
extends ShotsItemHandler {
    private static final int cE = 59151;
    private static final Skill[] a = new Skill[]{null, null, null, null, null, null, null, SkillTable.getInstance().getInfo(17819, 2), SkillTable.getInstance().getInfo(17820, 2), SkillTable.getInstance().getInfo(17821, 2)};
    ...
    public boolean useItem(Playable playable, ItemInstance itemInstance, boolean bl) {
        ...
        if (itemInstance2.getChargedSpiritshot() == 2) {
            return false;
        }
        ...
        if (grade2 != grade && !ArrayUtils.contains((int[])Config.ALT_UNIVERSAL_BLESSED_SPIRIT_SHOTS, (int)n)) {
            if (bl2) {
                return false;
            }
        ...
        itemInstance2.setChargedSpiritshot(2);
        player.sendPacket((IStaticPacket)SystemMsg.YOUR_SPIRITSHOT_HAS_BEEN_ENABLED);
        for (Skill skill : BlessedSpiritShot.a(itemInstance, player)) {
    ...
    public final int[] getItemIds() {
        ...
        for (ItemTemplate itemTemplate : ItemHolder.getInstance().getAllTemplates()) {
            if (itemTemplate == null || itemTemplate.getItemType() != EtcItemTemplate.EtcItemType.BLESSED_SPIRITSHOT) continue;
            cArrayIntSet.add(itemTemplate.getItemId());
        }
        cArrayIntSet.addAll((IntCollection)new CArrayIntList(Config.ALT_UNIVERSAL_BLESSED_SPIRIT_SHOTS));
